...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for sent in sents:
            new_sent = []
            for word in sent:
                new_word =[self.start_of_word] + [self.char2id[c] for c in word] + [self.end_of_word]
                new_sent.append(new_word)
            word_ids.append(new_sent)
        return word_ids
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        sentences = pad_sents_char(self.words2charindices(sents),self.char2id['<pad>'])
        sents_var = torch.tensor(sentences,device=device).permute([1,0,2])
        return sents_var        
