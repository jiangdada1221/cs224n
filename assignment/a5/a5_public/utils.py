...
def pad_sents_char(sents, char_pad_token):
    ...
    for i in range(len(sents)):
        for j in range(len(sents[i])):
            word = sents[i][j]
            sents[i][j] = sents[i][j] + [char_pad_token] * (max_word_length - len(sents[i][j])) if max_word_length > len(sents[i][j]) else sents[i][j][:max_word_length]
        sents[i] = sents[i] + (max_len-len(sents[i])) * [pad_word]
    sents_padded = sents
    return sents_padded
